...
AsyncNotifierProvider<CustomerNotifier, List<Customer>>(CustomerNotifier.new);
...
class CustomerNotifier extends AsyncNotifier<List<Customer>> {
  ...
  Future<void> _loadSortPreferences() async {
    ...
    _sortField = prefs.getString('customerSortField') ?? 'name';
    _sortAsc = prefs.getBool('customerSortAsc') ?? true;
  }

  Future<void> _saveSortPreferences() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('customerSortField', _sortField);
    await prefs.setBool('customerSortAsc', _sortAsc);
  }

  Future<List<Customer>> _fetchPage({bool reset = false}) async {
    if (reset) {
      _currentPage = 1;
      _hasMore = true;
    }
